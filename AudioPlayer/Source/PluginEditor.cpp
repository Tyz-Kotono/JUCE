...
void AudioPlayerAudioProcessorEditor::paint (Graphics& g)
{
...
//==============================================================================
void AudioPlayerAudioProcessorEditor::startOrStop()
{
...
void AudioPlayerAudioProcessorEditor::updateFollowTransportState()
{
    thumbnail->setFollowsTransport (followTransportButton.getToggleState());
}

void AudioPlayerAudioProcessorEditor::selectionChanged()
{
    audioProcessor.transportSourceCreator.requestTransportForURL(URL (fileTreeComp.getSelectedFile()));
}

void AudioPlayerAudioProcessorEditor::fileClicked (const File&, const MouseEvent&)          {}
void AudioPlayerAudioProcessorEditor::fileDoubleClicked (const File&)                       {}
void AudioPlayerAudioProcessorEditor::browserRootChanged (const File&)                      {}

void AudioPlayerAudioProcessorEditor::changeListenerCallback (ChangeBroadcaster* source)
{
    ...
    if( audioProcessor.sourceHasChanged.compareAndSetBool(false, true) )
    {
        ...
        if( hasValidSource )
        {
            if( src.get() != activeSource.get() )
            {
                ...
                AudioPlayerAudioProcessor::refreshCurrentFileInAPVTS(audioProcessor.apvts, src->currentAudioFile);
                activeSource = src;
